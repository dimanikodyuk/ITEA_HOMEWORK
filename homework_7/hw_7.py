...
class Departments():
    ...
    __insert_data_log = '''INSERT INTO log(created_dt, type, comment) VALUES(?,?,?);'''
    __insert_data_dep = '''INSERT INTO departments(department_name) VALUES(?);'''
    __update_data_dep = '''UPDATE departments SET department_name = ? WHERE department_id = ?;'''
    __delete_data_dep = '''DELETE FROM departments WHERE department_id = ?;'''
    ...
    def create_dep(self):
        ...
        if res is None:
            cr_dep = conn.cursor()
            cr_dep.execute(Departments.__insert_data_dep, (self.dep_name,))
            conn.commit()
            cr_dep.close()
            ...
            # Добавление лога
            Departments.create_log("create_dep",res_text)
        else:
    ...
    def delete_dep_by_id(p_department_id):
        del_dep = conn.cursor()
        del_dep.execute(Departments.__delete_data_dep, (p_department_id,))
        conn.commit()
        del_dep.close()
        #with conn:
        ...
        Departments.create_log("delete_dep_by_name", res_text)
